...
function renderStats() {
  ...
  const rankingMarkup = items => items.map((item, index) => `
    <button class="ranking-row ranking-link" data-state-open="${item.state}" aria-label="${item.state}: Fortschritt und Städte anzeigen">
  ...
  els.areaStateRanking.innerHTML = rankingMarkup(ranked.filter(item => !CITY_STATES.has(item.state)));
